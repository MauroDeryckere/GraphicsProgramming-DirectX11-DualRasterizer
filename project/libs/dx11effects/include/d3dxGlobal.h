...
#define DPF(level, str) { OutputDebugString(str); };
...
public:
    ~CEffectVectorOwner<T>()
    {
        ...
        for (size_t i=0; i< CEffectVector<T>::m_CurSize; i++)
            SAFE_DELETE(((T**)CEffectVector<T>::m_pData)[i]);

        SAFE_DELETE_ARRAY(CEffectVector<T>::m_pData);
    }
    ...
    void Empty()
    {
        ...
        CEffectVector<T>::m_CurSize = 0;
        CEffectVector<T>::m_hLastError = S_OK;
    }

    void Delete(_In_ uint32_t index)
    {
        assert(index < CEffectVector<T>::m_CurSize);

        SAFE_DELETE(((T**)CEffectVector<T>::m_pData)[index]);
...
public:
    ...
    void Cleanup()
    {
    	CEffectHashTable<T, pfnIsEqual>::CleanArray();
        CEffectHashTable<T, pfnIsEqual>::m_NumHashSlots = 0;
        CEffectHashTable<T, pfnIsEqual>::m_NumEntries = 0;
    }
    ...
    // Unsure if this works, if using this funcion be cautious
    ...
    HRESULT AddValueWithHash(_In_ T Data, _In_ uint32_t Hash)
    {
        ...
        //TODO get this assrt to work
		//assert(CEffectHashTable<T, pfnIsEqual>::m_NumHashSlots > 0);

    	typename CEffectHashTable<T, pfnIsEqual>::SHashEntry *pHashEntry;
        uint32_t index = Hash % CEffectHashTable<T, pfnIsEqual>::m_NumHashSlots;

        void* rawMemory = m_pPrivateHeap->Allocate(sizeof(CEffectHashTable<T, pfnIsEqual>::SHashEntry));
        // Use placement new to construct the object in the allocated memory
        pHashEntry = new (rawMemory) typename CEffectHashTable<T, pfnIsEqual>::SHashEntry;

        VN(pHashEntry)
        pHashEntry->pNext = CEffectHashTable<T, pfnIsEqual>::m_rgpHashEntries[index];
        pHashEntry->Data = Data;
        pHashEntry->Hash = Hash;
        CEffectHashTable<T, pfnIsEqual>::m_rgpHashEntries[index] = pHashEntry;

        ++CEffectHashTable<T, pfnIsEqual>::m_NumEntries;
